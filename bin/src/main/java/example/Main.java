...
public class Main {
  ...
  private static void sqlOperations(String driver, String conn)
  {
    ...
    for (int i : new int[]{8,9})
      opt.operation("0"+i, SQLOPT.VIEW);

    printTables(opt, conn);
  }

  public static void main(String[] args) {
    PropertyConfigurator.configure("/app/src/main/resources/log4j.xml");
    String databaseName = "test";

    // Create a variable for the connection string.
    String connectionStr = "jdbc:derby:memory:" + databaseName + ";create=true";

    sqlOperations("org.apache.derby.jdbc.EmbeddedDriver", connectionStr);
